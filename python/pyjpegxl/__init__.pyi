...
def jpeg_read_to_numpy(path: str | os.PathLike) -> tuple[JpegInfo, "np.ndarray[tuple[int, int, int], np.dtype[np.uint8]]"]:
...
async def async_encode(
    ...
    lossless: bool = False, quality: float = 1.0,
    speed: EncoderSpeed = EncoderSpeed.Squirrel, num_channels: int = 4,
    exif: bytes | None = None, xmp: bytes | None = None,
) -> bytes: ...
async def async_decode_to_numpy(data: bytes) -> tuple[Metadata, "np.ndarray[tuple[int, int, int], np.dtype[np.uint8]]"]: ...
...
async def async_read_to_numpy(path: str | os.PathLike) -> tuple[Metadata, "np.ndarray[tuple[int, int, int], np.dtype[np.uint8]]"]: ...
async def async_write(
    path: str | os.PathLike, data: bytes, width: int, height: int, *,
    lossless: bool = False, quality: float = 1.0,
    speed: EncoderSpeed = EncoderSpeed.Squirrel, num_channels: int = 4,
    exif: bytes | None = None, xmp: bytes | None = None,
) -> int: ...
...
async def async_jpeg_encode(
    data: bytes, width: int, height: int, *,
    quality: int = 95, num_channels: int = 3,
) -> bytes: ...
async def async_jpeg_decode_to_numpy(data: bytes) -> tuple[JpegInfo, "np.ndarray[tuple[int, int, int], np.dtype[np.uint8]]"]: ...
async def async_jpeg_encode_from_numpy(
    array: "npt.NDArray[np.uint8]", *, quality: int = 95,
) -> bytes: ...

async def async_jpeg_read(path: str | os.PathLike) -> tuple[JpegInfo, bytes]: ...
async def async_jpeg_read_to_numpy(path: str | os.PathLike) -> tuple[JpegInfo, "np.ndarray[tuple[int, int, int], np.dtype[np.uint8]]"]: ...
